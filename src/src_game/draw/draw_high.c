...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/07/22 22:56:00 by nvienot           #+#    #+#             */
/*   Updated: 2019/07/22 23:03:47 by nvienot          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "doom.h"

void	do_normal_high_line(t_draw *d, t_env *w, t_work *work, t_texture *text)
{
...
void	do_special_high_line(t_draw *d, t_env *w, t_work *work, t_texture *text)
{
...
void	draw_high_line_t(int x, t_env *w, t_work *work, t_texture *text)
{
	...
	if (d.y2 > d.y1)
	{
		if (vmid(work->ytop[x], 0, HEIGHT - 1) == d.y1
			...
			do_special_high_line(&d, w, work, text);
		else
			do_normal_high_line(&d, w, work, text);
	}
}
